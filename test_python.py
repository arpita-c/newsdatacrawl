...
def writeFrequencyCountDb(cursor,month,token,tokenlen):
    
    querySql="INSERT INTO `NewsDataAnalysis`.`FrequencyTable` (`month`, `token`, `count`) VALUES ('"+ \
                month+"','"+ token+"','"+tokenlen+"');"
...
if __name__ == '__main__':
    ...
    db=client.TelegraphData_December_Test
    ...
    finder = BigramCollocationFinder.from_words(globaltokenlist)
    finder.apply_word_filter(lambda w: w in ('.', ',','?','/'))
    
    trigram_finder=TrigramCollocationFinder.from_words(globaltokenlist)
    trigram_finder.apply_word_filter(lambda w: w in ('.', ',','?','/'))

    print "Bigram"
    print finder.nbest(bigram_measures.pmi, 300) 
    print "Trigram"
    print trigram_finder.nbest(trigram_measures.pmi, 300)
